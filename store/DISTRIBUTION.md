...
- Current upload artifact: `dist/store/PromptDirector-1.24.0-Chrome-Web-Store.zip`. Keep this artifact local for Web Store submission. The sole GitHub extension installation package is `PromptDirector-1.24.0.zip`, with stable identity and the local updater.
...
- `webRequest`: Observe media response URLs and content types on already authorized sites to populate tab-specific video badges and capture candidates. No request modification, credential collection or browsing-history upload. Cross-origin media still requires host access.
