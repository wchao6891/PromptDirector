...
def main() -> None:
    with extension_session("prompt-director-multimodel-routing-") as run:
        ...
        expect(text_model.locator('option[value="deepseek-v4-flash"]')).to_have_text("DeepSeek Flash")
        expect(image_model.locator('option[value="deepseek-v4-flash-vision-exp"]')).to_have_text("DeepSeek Flash")
        expect(image_model.locator('option[value="deepseek-v4-flash"]')).to_have_count(1)
