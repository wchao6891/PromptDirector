...
def main() -> None:
    with extension_session("prompt-director-page-capture-") as run:
        ...
        for selector in ['article', '#appendix']:
            fixture.bring_to_front()
            collector.evaluate("() => document.querySelector('#add-selection').click()")
            expect(fixture.locator('#promptdirector-content-picker')).to_be_attached(timeout=8000)
            fixture.locator(selector).evaluate("element => element.click()")
            expect(fixture.locator('#promptdirector-content-picker')).to_have_count(0)
            expect(collector.locator('#add-selection')).to_be_enabled()
        confirmed_article = collector.locator(".page-capture-item.confirmed .page-capture-article")
        collector.locator('.page-capture-preview-details').first.evaluate('node=>node.open=true')
        expect(confirmed_article).to_contain_text("downloadable production notes", timeout=8000)
        row=confirmed_article.locator('.page-capture-text-row').filter(has_text='A second paragraph')
        row.get_by_role('button',name='移除',exact=True).click()
        expect(confirmed_article).not_to_contain_text("A second paragraph")
        collector.locator("#page-capture-undo-region").click()
        expect(confirmed_article).to_contain_text("A second paragraph")
        confirmed_article.locator('.page-capture-text-row').filter(has_text='A second paragraph').get_by_role('button',name='移除',exact=True).click()
        expect(confirmed_article).not_to_contain_text("A second paragraph")
        expect(collector.locator("#page-capture-media-review-list .page-capture-media-review-item")).to_have_count(6)
        expect(collector.locator("#page-capture-media-review-status")).to_contain_text("6 项媒体")
        expect(collector.locator("#page-capture-save")).to_be_enabled()
