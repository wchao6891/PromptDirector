...
def assert_new_install_defaults() -> None:
    with extension_session("prompt-director-ai-registry-new-") as run:
        ...
        library.locator('[data-ai-routing-tab="providers"]').click()
        more = library.locator('#ai-provider-list .ai-more-providers')
        expect(more.locator('summary')).to_have_text('连接其他服务')
        assert not more.evaluate('(node) => node.open'), 'Unused connections stay collapsed on a fresh install'
        more.locator('summary').click()
        expect(more.locator('.ai-provider-row')).to_have_count(11)
        expect(more.get_by_role('button', name='配置 · Kimi', exact=True)).to_be_visible()
...
def main() -> None:
    ...
    with extension_session("prompt-director-ai-registry-", viewport={"width": 1280, "height": 900}) as run:
        ...
        expect(library.locator("#ai-provider-list")).to_contain_text("自定义图片生成服务")
        ...
        expect(library.locator("#ai-settings-status")).to_have_text("已保存")
        ...
        expect(library.locator("#vision-settings-status")).to_have_text("已保存")
        ...
        expect(dialog.locator("#promptdirector-app-dialog-providerEditor option", has_text="自定义兼容服务 · 自定义图片生成服务")).to_have_count(1)
        ...
        expect(analysis_key).to_be_hidden()
        ...
        library.locator('[data-ai-routing-tab="tasks"]').click()
        generation_task = library.locator("#ai-assignment-list .ai-assignment-row").filter(has=library.get_by_text("图片生成", exact=True))
        ...
        image_task = library.locator("#ai-assignment-list .ai-assignment-row", has_text="图片分析")
        expect(image_task).to_contain_text("自定义图片生成服务 · gpt-5.6-terra")
