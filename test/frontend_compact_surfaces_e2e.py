...
with extension_session('pd-six-surfaces-', viewport={'width':1440,'height':960}) as session:
 ...
 for width,height,locale,theme in [(1440,960,'zh-CN','dark'),(390,844,'en','light')]:
  ...
  centers=page.evaluate("""()=>{const a=document.querySelector('#batch-status-badge').getBoundingClientRect(),b=document.querySelector('#preview-analysis-batch').getBoundingClientRect();return Math.abs(a.top+a.height/2-b.top-b.height/2)}""")
  assert centers<=1,centers
 page.set_viewport_size({'width':1440,'height':960});page.locator('#settings-close').click()
 page.locator('#select-cases').click();page.locator('#selection-select-filtered').click();page.locator('#share-export').click()
